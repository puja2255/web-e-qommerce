...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setSlide((current) => (current + 1) % activeBanners.length),
      5000
    );
    ...
    <div className="stack" style={{ gap: 28 }}>
      {currentBanner ? (
        <section
          ...
          style={{
            backgroundImage: `linear-gradient(90deg, rgba(13,11,7,.82), rgba(13,11,7,.34)), url(${currentBanner.imageUrl})`,
          }}
        >
          <div className="home-banner__content">
            <span className="eyebrow">
              <Sparkles size={14} /> Promo Golden Store
            </span>
            <h1>{currentBanner.title}</h1>
            <p>{currentBanner.subtitle}</p>
            <Link href={currentBanner.link || "/products"} className="button">
          ...
          {activeBanners.length > 1 ? (
            <>
              <button
                ...
                onClick={() => setSlide((slide - 1 + activeBanners.length) % activeBanners.length)}
              ...
              <button
                ...
                onClick={() => setSlide((slide + 1) % activeBanners.length)}
      ...
      <section className="hero">
        <div className="hero-panel">
          <div className="eyebrow">
            <Sparkles size={14} /> Golden Store
          </div>
          <h1>Belanja mudah, cepat, dan aman dalam satu tempat.</h1>
          <p>
            Golden Store adalah e-commerce katalog lengkap dengan harga terbaik. Temukan produk yang
            dibutuhkan tanpa ribet.
          </p>
          <div className="hero-actions">
            ...
            <a
              ...
              rel="noreferrer"
            >
              <MessageCircleMore size={16} /> Tanya WA
            </a>
          </div>
        </div>
        <div className="hero-art panel">
          <div className="hero-art-card">
            <h3 style={{ fontSize: "1.8rem", marginTop: 18, marginBottom: 8 }}>
              Belanja jadi lebih praktis.
            </h3>
            <p className="muted">
              Semua dibuat agar pelanggan mudah menemukan produk yang diinginkan dan langsung
              bertransaksi tanpa hambatan.
            </p>
          </div>
          <div className="muted-box">
            Anda bisa menghubungi admin langsung via WhatsApp untuk tanya stok, detail produk, atau
            bantuan order.
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="section-title">
          <div>
            <div className="eyebrow">
              <Store size={14} /> Featured products
            </div>
        ...
        {/* Carousel Produk dengan Pembatas Tinggi Yang Dilepas (Height Auto) */}
        <div className="featured-carousel" style={{ height: "auto", minHeight: "260px", padding: "8px 0" }}>
          <div
