...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || request.cookies.get("golden_google_state")?.value !== stateHash(state)) return NextResponse.redirect(new URL("/account?error=google_failed", origin), 302);
  try {
    ...
    // Nama dari Google dipakai saat akun dibuat. Setelah pengguna mengubah
    // nama profilnya, jangan timpa lagi setiap kali ia masuk dengan Google.
    const user = await prisma.user.upsert({ where: { email: String(profile.email).toLowerCase() }, update: { googleId: profile.sub }, create: { email: String(profile.email).toLowerCase(), name, googleId: profile.sub, role: "CUSTOMER" } });
    const customer = { id: user.id, name: user.name, email: user.email, phone: user.phone ?? "" }; const response = NextResponse.redirect(new URL(next.startsWith("/") ? next : "/account", origin), 302); response.cookies.set(customerCookie(signCustomerSession(customer))); response.cookies.delete("golden_google_state"); return response;
  } catch { return NextResponse.redirect(new URL("/account?error=google_failed", origin), 302); }
}
