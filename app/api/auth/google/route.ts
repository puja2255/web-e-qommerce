...
export async function GET(request: Request) {
  ...
  if (!clientId) return NextResponse.redirect(new URL("/account?error=google_not_configured", origin), 302);
  ...
  response.cookies.set("golden_google_state", stateHash(state), { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/", maxAge: 600 });
  return response;
}
