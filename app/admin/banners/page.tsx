...
export default function AdminBannersPage() {
  ...
  return <AdminShell title="Banner halaman utama" description="Atur gambar, teks, tautan, dan urutan banner slider di beranda." action={<button className="button" onClick={reset}><Plus size={16} /> Banner baru</button>}>
    <section className="panel"><h2 style={{ marginTop: 0 }}>{editingId ? "Edit banner" : "Tambah banner"}</h2><form className="stack" onSubmit={submit}>
      ...
      <div className="field"><label>URL gambar banner atau file</label><input className="input" type="url" value={draft.imageUrl} onChange={(event) => setDraft({ ...draft, imageUrl: event.target.value })} placeholder="https://..." /><input className="input" type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setDraft((current) => ({ ...current, imageUrl: String(reader.result) })); reader.readAsDataURL(file); }} /></div>
