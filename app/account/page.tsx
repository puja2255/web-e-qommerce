...
import { CheckCircle2, Clock3, KeyRound, LogOut, MapPin, PackageCheck, PackageOpen, Star, Truck, Upload, UserRound } from "lucide-react";
...
export default function AccountPage() {
  ...
  const { customerSession, customerAddresses, orders, registerCustomer, loginCustomer, requestOtp, updateCustomerProfile, logoutCustomer, saveCustomerAddress, deleteCustomerAddress, uploadPaymentProof, refreshData } = useGoldenStore();
  const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [otp, setOtp] = useState(""); const [mode, setMode] = useState<"login" | "register">("login"); const [tab, setTab] = useState<"profile" | "addresses" | "orders">("profile"); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  const next = typeof window === "undefined" ? "/account" : new URLSearchParams(window.location.search).get("next") || "/account";
  const register = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); const result = mode === "login" ? await loginCustomer(email, password) : await registerCustomer({ name, email, password, otp }); setBusy(false); if (!result.ok) { setError(result.message ?? "Akun tidak dapat diproses."); return; } router.push(next); };
  const sendOtp = async (purpose: "REGISTER" | "PROFILE") => { const target = purpose === "PROFILE" ? customerSession?.email ?? "" : email; const result = await requestOtp(target, purpose); setError(result.ok ? (result.debugCode ? `Mode lokal — OTP: ${result.debugCode}` : "OTP telah dikirim ke email.") : (result.message ?? "OTP gagal dikirim.")); };
  const saveProfile = async (event: FormEvent) => { event.preventDefault(); const result = await updateCustomerProfile({ name: name || undefined, password: password || undefined, otp }); setError(result.ok ? "Profil diperbarui." : (result.message ?? "Profil gagal diperbarui.")); if (result.ok) { setPassword(""); setOtp(""); } };
  ...
  if (!customerSession) return <section className="panel auth-shell" style={{ maxWidth: 620, margin: "0 auto" }}><div className="eyebrow"><UserRound size={14} /> Akun pembeli</div><h1>{mode === "login" ? "Masuk ke akunmu" : "Buat akun untuk belanja lebih mudah"}</h1><p className="muted">Login cukup pakai nama atau email dan password.</p><div className="auth-switch"><button type="button" className={mode === "login" ? "active" : ""} onClick={() => { setMode("login"); setError(""); }}>Masuk</button><button type="button" className={mode === "register" ? "active" : ""} onClick={() => { setMode("register"); setError(""); }}>Daftar</button></div><form className="stack" onSubmit={register}>{mode === "register" ? <div className="field"><label>Nama lengkap (unik)</label><input className="input" value={name} onChange={(e) => setName(e.target.value)} minLength={3} required /></div> : null}<div className="field"><label>{mode === "login" ? "Nama atau email" : "Email"}</label><input className="input" type={mode === "login" ? "text" : "email"} value={email} onChange={(e) => setEmail(e.target.value)} required /></div><div className="field"><label>Password</label><input className="input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required /></div>{mode === "register" ? <div className="field"><label>OTP email</label><div className="row-actions"><input className="input" value={otp} onChange={(e) => setOtp(e.target.value)} maxLength={6} required /><button className="button-outline" type="button" onClick={() => void sendOtp("REGISTER")}>Kirim OTP</button></div></div> : null}{error ? <p className="review-feedback">{error}</p> : null}<button className="button" type="submit" disabled={busy}><UserRound size={16} />{busy ? "Memproses..." : mode === "login" ? "Masuk" : "Buat akun"}</button></form></section>;
